...
/**
 * @class Simulation_universe, inheritance from Simulation class
 *
 * attributes :
 * @private temperature : current temperature of the universe.
 * @private hubble_cst : current value of the Hubble-LeMaître constant.
 * @private matter_parameter : current value of the matter density parameter.
 * @private dark_energy : object containing current value of dark energy density parameter, value of w_0 and value of w_1.\
 * 	Note : When w_0 = -1 and w_0 = 0, the universe is equivalent to his counterpart with only a cosmologic constant.
 * @private constants : contains the value of the physics constants defined for the universe.
 * @private has_cmb : Has Cosmic Microwave Background (CMB).
 * @private has_neutrino : self explanatory.
 * @private is_flat : Forcing the curvature density parameter to 0.
 * 
 * methods names :
...
export class Simulation_universe extends Simulation {
    ...
    /**
     * @description formula 1/(1 + x) * sqrt(1 / F)
    ...
    /**
     * @description formula 1/(1 + x) * 1/sqrt(F) with the substitution x = y/(1 - y), to be used with simpson method to compute duration.
     * @param Simu object in witch method is applied, permit to use this function with simulation method
     * @param y variable
